...
ncaa_team_schedules = CSV.open("csv/pbptest.csv","r",{:col_sep => "\t", :headers => TRUE})
...
game_ids.each_slice(gpt).with_index do |ids,i|
  ...
  threads << Thread.new(ids) do |t_ids|
    ...
    t_ids.each_with_index do |game_id,j|
      ...
      game_url = 'http://anype.com/SURF/http://stats.ncaa.org/game/play_by_play/%d&ANYPE_SUBMIT=0' % [game_id]
        ...
        row.xpath('td').each_with_index do |element,i|
          ...
          when 0
            ...
            if not(link.nil?)
              ...
              team_url = link_url.split("SURF/")[1]
